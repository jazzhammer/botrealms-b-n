  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      if (search.length > 0) {
        ...
        sql = `select * from realm where name like '%${search}%'`;
    ...
    try {
      ...
      let selectResult = await client.query(`SELECT * FROM realm where name = '${name}'`);
      const alreadyRows = selectResult.rows;
      if (alreadyRows?.length > 0) {
        created = alreadyRows[0];
      } else {
        const createResult = await client.query(`INSERT INTO realm (name) values ('${name}')`);
        selectResult = await client.query(`SELECT * FROM realm where name = '${name}'`);
    ...
    try {
      ...
      const body = await readRequestBody(req);
      bodyJson = JSON.parse(body);
      const {realm_id, name} = bodyJson;
      const updateResult = await client.query(`UPDATE realm set name = '${name}' where realm_id = '${realm_id}'`);
      const selectResult = await client.query(`SELECT * FROM realm where realm_id = '${realm_id}'`);
      const selectRows = selectResult.rows;
      if (selectRows) {
